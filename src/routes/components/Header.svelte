<script>
	...
	function toggleUserHeader() {
		...
		console.log('O0', userMenu.classList.contains('opacity-0'));
		console.log('O100', userMenu.classList.contains('opacity-100'));
...
<header class="fixed w-full mt-6 top-[0]">
	<div class="relative flex flex-nowrap justify-between w-10/12 mx-auto">
		...
		{#if user.isLogOn}
			<button class="rounded-full bg-gray-5 w-[36px] h-[36px]" on:click={toggleUserHeader}>
				<div class="bg-[url('{user.avatar}')] rounded-full h-full w-full" />
			</button>
		{/if}
		...
		<div on:click={toggleNotificationHeader}>
			...
			{#if user.hasNotification}
				<div
					class="absolute bg-red-1 rounded-full animate-ping"
					style="height: 12px; width: 12px; top: 24px; right: 0;"
				/>

				<div
					class="absolute bg-red-2 rounded-full"
					style="height: 12px; width: 12px; top: 24px; right: 0;"
				/>
			{/if}
		</div>
